...
token_text = open("C:\\Users\\qsc14\\Desktop\\discord\\mw_game_bot_token.txt", 'r')
...
@bot.command()
async def 패치노트(ctx):
    embed = discord.Embed(title='📖 머웅 봇 v2.1 패치노트', color=random.choice(colors))
    # 📕📘📙📗
    embed.add_field(name='📕 1. 이벤트 **팀** 점수 기능 활성화', value='`팀점수 관련 추가 완료!`', inline=False)
    embed.add_field(name='📘 2. `!롤멤버초기화` 추가', value='`!롤멤버초기화` `명령어를 통해 현재 생성된 롤파티를 초기화 할 수 있습니다.`', inline=False)
...
async def 팀점수추가(ctx, n):# !팀점수추가 (x팀) > 몇점추가할래? > 대답(int) > 적용
    ...
    if team_score_list == []: # 팀나누기 없이 팀점수추가?
        ...
        await ctx.send(team_score_list)
...
async def 프로필(ctx, char_name):
    ...
    '''
    char_main = driver.find_element_by_xpath('//*[@id="char-card-body"]/div[1]/div')
    char_equip = driver.find_element_by_xpath('//*[@id="char-card-body"]/div[2]/div[1]')
    char_jewel = driver.find_element_by_xpath('//*[@id="char-card-body"]/div[2]/div[2]')
    char_engrave = driver.find_element_by_xpath('//*[@id="char-card-body"]/div[2]/div[3]')
    char_card = driver.find_element_by_xpath('//*[@id="char-card-body"]/div[2]/div[4]')
    char_skill = driver.find_element_by_xpath('//*[@id="char-card-body"]/div[2]/div[5]')
    '''

    '''
    driver.implicitly_wait(1)
    char_main.screenshot('screen0.png')
    driver.implicitly_wait(1)
    char_equip.screenshot('screen1.png')
    driver.implicitly_wait(1)
    char_jewel.screenshot('screen2.png')
    driver.implicitly_wait(1)
    char_engrave.screenshot('screen3.png')
    driver.implicitly_wait(1)
    char_card.screenshot('screen4.png')
    driver.implicitly_wait(1)
    char_skill.screenshot('screen5.png')
    '''
    
    
    """
    # 메인 이미지
    image_main = discord.File("screen0.png", filename = "main.png")
    page0 = discord.Embed(title='🍀 프로필1', color=random.choice(colors))
    page0.set_image(url="attachment://main.png")
    page0.set_author(name=ctx.author.display_name, icon_url=ctx.author.avatar_url)
    # 장비 이미지
    ...
    message = await ctx.send(embed = page0, file = image_main)
    ...
    while True: # 리액션에 따른 임베드 출력
        if str(reaction) == '💡':
            ...
            await ctx.send(embed=page0, file = image_main)

        elif str(reaction) == '📖':
            i = 1
            await message.delete()
            image_equip = discord.File("screen1.png", filename = "equip.png")
            page1 = discord.Embed(title='🍀 프로필2', color=random.choice(colors))
            page1.set_image(url="attachment://equip.png")
            page1.set_author(name=ctx.author.display_name, icon_url=ctx.author.avatar_url)
            message = await ctx.send(embed = page1, file = image_main)
            await message.add_reaction('💡')
            await message.add_reaction('📖')
            await message.add_reaction('✏️')
            ...
            i = 2
            await message.delete()
            image_engrave = discord.File("screen2.png", filename = "engrave.png")
            page2 = discord.Embed(title='🍀 프로필3', color=random.choice(colors))
            page2.set_image(url="attachment://engrave.png")
            page2.set_author(name=ctx.author.display_name, icon_url=ctx.author.avatar_url)
            message = await ctx.send(embed = page2, file = image_main)
            await message.add_reaction('💡')
            await message.add_reaction('📖')
            await message.add_reaction('✏️')
...
async def on_command_error(ctx, error):  # 커맨드 에러
    ...
    if isinstance(error, commands.CommandNotFound):
        # await ctx.send(f'그런 명령어는 없어. 만들어줄까? <@{mw_id}>')
